...
//template mat copy karna bhosdiwalo 
...
//********************************************************************************
void pmap(map<li,li>m ){for(auto q:m){ct<<q.ff<<" "<<q.ss<<el;}ct<<el;}
void pvecpair(vector<pair<li,li>> &v){for(auto q:v){ct<<q.ff<<" "<<q.ss<<el;}ct<<el;}
...
void printvec(vector<li> &v ){for (int i = 0; i < v.size(); ++i){ct<<v[i]<<" ";}ct<<el;}
void printvec2(vector<vector<li>> &v){for (int i = 0; i <v.size() ; ++i){for (int j = 0; j < v[i].size(); ++j){ct<<v[i][j]<<sp;}ct<<el;}}
void take_input2(vector<vector<li>> &v , li n,li m){for (int i = 0; i <n ; ++i){vec temp;for (int j = 0; j < m; ++j){li x;cin>>x;temp.pb(x);}v.pb(temp);}}
bool cmp(pair<li,li>a,pair<li,li>b){if (a.ff!=b.ff){return a.ff<b.ff;}return a.ss>b.ss;}
void padj(vec adj[],int n){for (int i = 0; i < n; ++i){ct<<i<<sp<<":"<<sp;for(auto t:adj[i]){ct<<t<<sp;}ct<<el;}ct<<el;}
