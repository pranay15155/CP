...
li f(li limit ,vec & positive,vec &negative,li m){
	li p=0,n=0;
	li sum=0;//current sum
	li mini=0;//minimum prefix
	li ans=0;//maximum subarray sum
	spc();
	d(limit);
	vec temp;
	for (int i = 0; i <m ; ++i)
	{
		if (positive.size() && sum+positive[p]-mini>limit || p==positive.size())
		{
			if (n==negative.size())
			{
				return 0;
		...
		else if (positive.size() && sum+positive[p]-mini<=limit && p<positive.size())
		{
			temp.pb(positive[p]);
			sum+=positive[p];
			mini=min(mini,sum);
...
void pranay_prasoon(){
	...
	while(low<=high){
		li mid=((low+high)/2);
		if(f(mid,positive,negative,n)==1){
			ans=mid;
			high=mid-1;
		}
		else{
